...
/**
 * This class will check the webdav for the resources if the
 * webDavResourceLoader has a value. If the resource is not found the
 * ReloadableResourceBundleMessageSource default functionality is performed.
 *
 * @see org.springframework.context.support.ReloadableResourceBundleMessageSource
 * @author Peterv
 */
public class WebDavReloadableResourceBundleMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
	private final static Logger logger = LoggerFactory.getLogger(WebDavReloadableResourceBundleMessageSource.class);
	...
	private ResourceLoader webDavResourceLoader;
	...
	protected PropertiesHolder refreshProperties(String filename, PropertiesHolder propHolder) {
		...
		try {
			if (this.webDavResourceLoader != null) {
				// get the resource from the webdav
				resource = this.webDavResourceLoader.getResource(filename + PROPERTIES_SUFFIX);
				if (!resource.exists()) {
					resource = this.webDavResourceLoader.getResource(filename + XML_SUFFIX);
				}
			}
		} catch (Exception e) {
			// log an issue but no crash...
			logger.error("Unable to get resource from webdav [" + filename + "] - neither plain properties nor XML " + e.getMessage());
		}
		if (!resource.exists()) {
		...
		if (resource.exists()) {
			...
			if (this.cacheMillis >= 0) {
				// Last-modified timestamp of file will just be read if
				// caching
				// with timeout.
				try {
	...
	public void setWebDavResourceLoader(ResourceLoader webDavResourceLoader) {
